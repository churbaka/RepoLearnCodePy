...
from 1 to max int value. This cipher rotates the letters of the alphabet (A to Z). The
encoding replaces each letter with the 1st to 25th next letter in the alphabet
(wrapping Z to A). So key 3 encrypts "HELLO" to "KHOOR", but key 20 encrypts "HELLO" to
"CZGGJ". This simple "monoalphabetic substitution cipher" provides almost no
...
if __name__ == '__main__':
    cipher = Caesar()
    key = 21
    message_encrypt = 'Secret message!'
    message_decrypt = cipher.encrypt(message=message_encrypt, key=int(key%25))
    message_received = cipher.decrypt(message=message_decrypt, key=(key%25))
    print(f'Your key: {key}')
    print(f'Your encrypted message: {message_decrypt}')
    print(f'Your decrypted message: {message_received}')
